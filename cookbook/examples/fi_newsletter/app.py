...
from assistant import get_fi_assisant  # type: ignore

nest_asyncio.apply()
st.set_page_config(
    page_title="SS Fixed Income AI",
    page_icon="📰",
)
st.title("Daily Newsletter Assistant 📰")
st.markdown("##### Created for internal use in Sucor Sekurtias FI only")
...
def main() -> None:
    ...
    if "auto_rag_assistant" not in st.session_state or st.session_state["auto_rag_assistant"] is None:
        ...
        auto_rag_assistant = get_fi_assisant(llm_model=llm_model)
